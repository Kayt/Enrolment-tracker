...
def Load_Data(file_name):
    data = genfromtxt(file_name, delimiter=',', skip_header=1, converters={
                      0: lambda s: str(s), 1: lambda s: str(s), 2: lambda s: str(s)})
    return data.tolist()
...
if __name__ == "__main__":
    ...
    try:
        file_name = "enrolments.csv"
        data = Load_Data(file_name)

        for i in data:
            logging.debug("....... Adding record {}".format(i))
            record = EnrolmentModel(i[0], i[1], i[2])
        ...
        logging.info(
            "*-------- Done Adding {0} records in {1} seconds ---------*".format(len(data), str(time()-t)))
